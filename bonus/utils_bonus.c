...
int	checkopen_size(char *filename, t_game *game)
{
	...
	if (fd < 0)
		print_err_exit("File open fail Error");
	line = get_next_line(fd);
	if (line == 0)
		print_err_exit("File empty Error");
	...
	while (line > 0)
	{
		if (line[ft_strlen(line) - 1] == '\n')
		{
			if (game->map_wid != (int)ft_strlen(line) - 1)
				print_err_exit("line align Error");
		}
...
void	checking_1d_arr(int idx, char *map, t_game *game)
{
	...
	if (game->player.x == -1 || game->player.y == -1)
		print_err_exit("no player Error");
	if (game->collection == 0)
		print_err_exit("no collection Error");
	if (game->exit_cnt != 1)
		print_err_exit("exit is not one");
}
